...
const EXPERIENCE_TYPE_LOOKUP = new Map(
  SHOWTIME_EXPERIENCE_TYPES.map((experienceType) => [experienceType.toLowerCase(), experienceType])
);

export function parseShowtimeExperienceTypes(values: Iterable<string>): ShowtimeExperienceType[] {
  const parsed = new Set<ShowtimeExperienceType>();

  for (const rawValue of values) {
    for (const value of rawValue.split(",")) {
      const experienceType = EXPERIENCE_TYPE_LOOKUP.get(value.trim().toLowerCase());
  ...
  return SHOWTIME_EXPERIENCE_TYPES.filter((experienceType) => parsed.has(experienceType));
}
...
export function showtimeMatchesExperienceTypes(
  ...
  return selectedTypes.some((selectedType) => {
    if (selectedType === "VIP") {
      return Array.from(formatTokens).some((token) => token === "vip" || token.startsWith("vip "));
    }
